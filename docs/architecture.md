...
関連: [../README.md](../README.md)（概要）／[audio-basics.md](audio-basics.md)（音の基礎）／[visualizer-basics.md](visualizer-basics.md)（描画の基礎＝シェーダ/WebGL等）／[features.md](features.md)（材料の一覧）
...
各フィールドの範囲・意味は [features.md](features.md) に一覧。
...
`npm run build` で両ホストを出力：`dist/`（拡張）と `dist-app/`（Electron）。

### A. 拡張（YouTube 重ね）

1. Chrome で `chrome://extensions` → デベロッパーモード ON
2. 「パッケージ化されていない拡張機能を読み込む」→ `dist/` を選ぶ
